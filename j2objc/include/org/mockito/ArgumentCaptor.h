 ...
 *don't do any type checks*</strong>, the generic signatures are only there to avoid casting in your code. If you want specific types, then you should do that the captured values. This behavior might change (type checks could be added) in a future major release. <p> There is an <strong>annotation</strong> that you might find useful: &#64;<code>Captor</code> <p> See the full documentation on Mockito in javadoc for <code>Mockito</code> class.
 - seealso: Captor
 @since 1.8.0
 */
...
/*!
 @brief Returns all captured values.Use it in case the verified method was called multiple times.
 <p>
  Example:  
 <pre class="code"><code class="java">
...
FOUNDATION_EXPORT OrgMockitoArgumentCaptor *new_OrgMockitoArgumentCaptor_init(void) NS_RETURNS_RETAINED;

FOUNDATION_EXPORT OrgMockitoArgumentCaptor *create_OrgMockitoArgumentCaptor_init(void);
